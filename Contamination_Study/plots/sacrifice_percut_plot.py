...
def SacVSVar_Data(rootfiles=[],var="nhits",nbins=10,vmin=10.0,vmax=100.0,dcmask=None):
    '''
    ...
    data.Draw("%s>>h_allevents(%i,%f,%f)"% (var,nbins,nmin,nmax),
            "","goff")
    ...
    for cut in plotmask:
        ...
        data.Draw("%s>>h_flagged(%i,%f,%f)" % (var,nbins,nmin,nmax),
                "isValid==1&&isCal==1&&((dcFlagged&%i)!=%i)" % (cutint,cutint),"goff")
        ...
        for i in xrange(h_cut_FracFlagged.GetNbinsX()):
            vardat =  vardat + (int(h_cut_FracFlagged.GetBinWidth(i) + h_cut_FracFlagged.GetBinLowEdge(i)),)
...
def SacVSVar_Plot(allcutsacs,topcuts=None):
    ...
    plt.xlabel(allcutsacs.var,fontsize=22)
